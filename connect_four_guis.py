...
class MainMenuGui:
    ...
    def __init__(self, scaling_factor: float = 1.0):
        ...
        self.main_menu_screen: Screen = Screen(
        ...
        title_text.position = (
            int(self.main_menu_screen.width / 2 - title_text.width / 2),
            int(self.main_menu_screen.height * 0.05),
        )
        self.game_object_id[title_text.name] = len(self.main_menu_screen.game_objects)
        self.main_menu_screen.game_objects.add(title_text)
        ...
        player_vs_player_button.position = (
            int(self.main_menu_screen.width / 2 - player_vs_player_button.width / 2),
            int(self.main_menu_screen.height * 0.25),
        )
        self.game_object_id[player_vs_player_button.name] = len(
            self.main_menu_screen.game_objects
        )
        self.main_menu_screen.game_objects.add(player_vs_player_button)
        ...
        player_vs_computer_button.position = (
            int(self.main_menu_screen.width / 2 - player_vs_computer_button.width / 2),
            int(self.main_menu_screen.height * 0.40),
        )
        self.game_object_id[player_vs_computer_button.name] = len(
            self.main_menu_screen.game_objects
        )
        self.main_menu_screen.game_objects.add(player_vs_computer_button)
        ...
        computer_vs_computer_button.position = (
            int(
                self.main_menu_screen.width / 2 - computer_vs_computer_button.width / 2
            ),
            int(self.main_menu_screen.height * 0.55),
        )
        self.game_object_id[computer_vs_computer_button.name] = len(
            self.main_menu_screen.game_objects
        )
        self.main_menu_screen.game_objects.add(computer_vs_computer_button)
        ...
        train_computer_button.position = (
            int(self.main_menu_screen.width / 2 - train_computer_button.width / 2),
            int(self.main_menu_screen.height * 0.70),
        )
        self.game_object_id[train_computer_button.name] = len(
            self.main_menu_screen.game_objects
        )
        self.main_menu_screen.game_objects.add(train_computer_button)
    ...
    def _event_check(self, event: Event) -> Codes:
        if event.type == pygame.QUIT:
            ...
            for game_object in self.main_menu_screen.game_objects:
    ...
    def start(self) -> Codes:
        ...
        while code is Codes.RUNNING:
            ...
            self.main_menu_screen.update()
            self.display.blit(
                self.main_menu_screen.screen,
                (self.main_menu_screen.x, self.main_menu_screen.y),
            )
            display.update()
