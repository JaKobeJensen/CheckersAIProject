...
class MinimaxConnectFourAI:
    ...
    _HEAT_MAP = [[10, 20, 30, 40, 30, 20, 10],
    ...
    def _minimax_rec(self, game_state: ConnectFour, depth: int, alpha: float, beta: float, maximizing_player: bool) -> int:
        ...
        if maximizing_player:
            ...
            for move in game_state.legal_moves():
                game_state_copy = copy.deepcopy(game_state)
                game_state_copy.player_move(move)
                value = max(value, self._minimax_rec(game_state_copy, depth - 1, alpha, beta, False))
            ...
            for move in game_state.legal_moves():
                game_state_copy = copy.deepcopy(game_state)
                game_state_copy.player_move(move)
                value = min(value, self._minimax_rec(game_state_copy, depth - 1, alpha, beta, True))
                beta = min(beta, value)
                if value <= alpha:
                    break
            return value
    ...
    def _heuristic_function(self, game_state: ConnectFour) -> float:
        ...
        for row_idx, row in enumerate(game_state.game_board):
            for col_idx, space in enumerate(row):
                heuristic_value += space * self._HEAT_MAP[row_idx][col_idx] * self._piece_color
